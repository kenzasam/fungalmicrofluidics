...
class ArduBridge_Shield():
    ...
    def pwm_init(self, dirPin, pwmCh):
        self.ardu.gpio.pinMode(dirPin,0) #dirPin as output
        self.ardu.gpio.digitalWrite(self.pwmCh[pwmCh],0) #pwm = 0%
    
    def pwmA(self, p):
            self.pwm(p, dirPin=2, pwmCh=0)

    def pwmB(self, p):
            self.pwm(p,dirPin=4, pwmCh=1)

    def pwm(self, p, dirPin, pwmCh):
        p = float(p)
        if p >= 0.0:
            di = 0
            if p > 100.0:
